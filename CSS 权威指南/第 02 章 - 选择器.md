...
|  [attribute~=value]  | [target~=flower] | 选择 title 属性包含单词 "flower" 的所有元素（单词以空格分隔） |
...
- CSS 关键字往往由空格分隔，**只有一种情况例外**。`h2 {font: large/150% sans-serif}` 斜线分隔了用来设置元素的字体大小和行高的两个关键字。
- **所有伪元素都必须放在出现该伪元素都选择器的最后面**。因此，如果写成 `p:first-line em` 就是不合法的。